...
slangc_path = os.path.join(
    package_dir, 'bin', 'slang', 'slangc'+executable_extension)
...
def loadModule(fileName, skipSlang=False, verbose=False, copts=[]):
    if verbose:
    ...
    if not(skipSlang and os.path.exists(cudaOutName)):
        result = subprocess.run([slangc_path, fileName, '-o', cudaOutName ] + copts, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        slangcOutput = result.stderr.decode('utf-8')
        if slangcOutput.strip():
            print(slangcOutput)
        if result.returncode != 0:
